...
#### signal_job - 发送 SIGTERM

向运行中的任务进程发送 SIGTERM 信号，但**不**终止任务。与 `terminate_job` 互补：

- `terminate_job` 是不可逆的硬取消，状态立刻置为 `Terminated`，进程没有协调窗口；
- `signal_job` 只是信号转发器，任务状态不变。装了 SIGTERM handler 的用户代码可以借此跑自定义收尾（保存中间结果 / 检查点、释放 GPU 显存与 NCCL 资源、关闭外部连接、刷新输出缓冲等）；进程响应信号自然退出后由服务端常规同步收敛到 `Success` 或 `Failed`。
...
**前置条件**：任务必须处于 `Running` 状态。其他状态会返回 409。

**典型用法**：手动终止前先 `signal_job` 发出温柔通知，观察日志看进程是否正在收尾，若不响应再 `terminate_job` 强杀。

> **Admin 特权**：与 `terminate_job` 一致，管理员可以向任何人的任务发送信号。
...
| `get_metric_streams(job_id)` | 列出任务的指标流 | `List[Dict]` |
| `get_metric_points(job_id, name, labels=, as_numpy=False, ...)` | 获取指标原始点 | `List[Dict]` 或 `(steps, values, times)` ndarray 三元组 |
...
| `signal_job(job_id)` | 向任务进程发送 SIGTERM（不终止任务，需用户代码自行处理） | 状态信息 |
...
magnus signal <ref>      # 向任务进程发送 SIGTERM（不终止任务）
...
向任务进程发送 SIGTERM 信号。**不会**终止任务，也不修改任务状态：仅作为信号转发器，给装了 SIGTERM handler 的用户代码自定义清理（NCCL teardown、CUDA context 释放、保存检查点等）的入口。
...
进程响应信号自然退出后，由服务端常规同步走 `Success` 或 `Failed`。如果用户代码忽略信号，任务继续运行；可以再用 `magnus job kill` 强制终止。

要求任务处于 `Running` 状态。

#### magnus job metric

列出任务的指标流，拉取原始数据点，或保存服务器渲染的图表。输出文件后缀决定格式：`.csv` / `.json` / `.yaml` 写原始数据，`.png` 写图表。

```bash
magnus job metric -1                                  # 列出指标流
magnus job metric -1 train.loss                       # 打印数据点（管道下输出 YAML）
magnus job metric -1 train.loss -o loss.csv          # 保存 CSV（labels 展开为 label_* 列）
magnus job metric -1 train.loss -o loss.png          # 保存服务器渲染 PNG
magnus job metric -1 train.loss --output loss.png    # --output 长格式别名
magnus job metric -1 train.loss --labels '{"global_rank":"0"}'
```

**选项**：
- `-o, --output`: 写入文件；后缀决定格式（.csv/.json/.yaml = 数据，.png = 图表）
- `--labels`: 按 labels 过滤（JSON 对象）
- `--step-domain`: 按 step_domain 过滤
- `--max-points`: 服务器端降采样目标（1..10000），默认 2000
- `-f, --format`: stdout 输出格式（yaml/json/table）
