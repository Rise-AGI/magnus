...
| `{work}/.magnus_user_root` | user script entry → cleanup | container bash (`echo $$`) | wrapper.py (SIGTERM handler) | PID of the bash running the user's entry_command; anchor of the Magnus user-root convention so the wrapper-side SIGTERM fan-out can locate the user subtree |
...
**SLURM path (signal)**: `scancel --signal=TERM --full <jobid>` — `--full` is mandatory, otherwise `scancel --signal` only targets non-batch steps and wrapper.py is the batch script started directly by sbatch (no srun). SLURM delivers the signal via `killpg` to the batch step's process group, not to every PID in the cgroup. Intermediate commands that call `setsid` / `setpgid` create their own process group and the user code beyond them falls out of the batch step pgrp — GNU `timeout` does this by default (so `timeout 20s python main.py` puts the python in `timeout`'s pgrp), and apptainer's starter and rootlesskit behave similarly. Magnus closes this gap with a small implicit protocol that is transparent to users:

- **Magnus user-root convention**: `.magnus_user_script.sh` (rendered by `_wrapper_template.py`) has `echo $$ > "$MAGNUS_HOME/workspace/.magnus_user_root"` as its first executable line — the bash that runs the user's `entry_command` writes its own PID into the `.magnus_user_root` marker in the workspace (bind-mounted to the host work-dir). apptainer does not create a new PID namespace by default, so `$$` is the host-visible PID. The marker is the "root of the user process tree" as Magnus defines it: everything fork-descended from this bash, no matter how many setpgid layers in between, is "the user's entry_command".
...
- **Wrapper-side fan-out**: `_on_sigterm` first records `_signaled[0] = True` (so the Phase 3 marker fallback can run even if the forward fails), then calls `_signal_user_subtree(SIGTERM, marker_path)`. That helper reads `.magnus_user_root` to learn the user-root PID, reads this job's cgroup.procs, walks /proc/<pid>/task/<pid>/children to BFS the descendants of user-root that are also in the cgroup, and sends SIGTERM to every PID in that subtree (the user-root itself plus all descendants). Whatever the user's `entry_command` spawned — main process, fork children, MPI workers, nested shells — receives the signal. apptainer starter / appinit / subprocess shell are ancestors of user-root rather than descendants, so they are not signaled and the container is not torn down; fuse-overlayfs / squashfuse_ll daemonize with PPid outside the cgroup, so they fall outside the BFS too. The whole helper is fail-open at the IO and per-kill level — any procfs read failure, missing marker (during the tiny startup race), or kill failure continues with what we have, and the handler never throws back to main.
- **User process**: inside the container the user processes inherit SIG_DFL for SIGTERM. Without a handler the wrapper-forwarded SIGTERM terminates them. With a `signal.signal(SIGTERM, …)` handler the user code runs its own teardown and decides its own exit code; writing `.magnus_result` from inside the handler lets `sys.exit(0)` count as success (see "State convergence" below). Behavior is identical across Python / C / Fortran / any POSIX runtime — the protocol delivers a plain SIGTERM, the user-side idiom is plain `sigaction(2)` semantics.
...
${MAGNUS_HOME}/workspace/.magnus_user_root   user-root PID marker (written by user script entry)
