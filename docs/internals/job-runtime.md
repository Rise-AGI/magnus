...
**SLURM path (signal)**: `scancel --signal=TERM --full <jobid>` — `--full` is mandatory, otherwise `scancel --signal` only targets non-batch steps and wrapper.py is the batch script started directly by sbatch (no srun). proctrack propagates the signal down the cgroup to every PID. The chain `outer bash (sbatch shebang) → wrapper.py → subprocess shell → apptainer → in-container bash → user process` is half SIG_IGN, half not, by design:

- **wrapper.py and below SIG_IGN** through POSIX exec inheritance: the sbatch batch script renders as `#!/bin/bash\ntrap '' TERM\n\nexec <wrapper>` so the outer bash sets SIG_IGN before any work and `exec` carries it into wrapper.py; wrapper.py main() then `signal.signal(SIGTERM, _on_sigterm)` (a plain handler that records the fact a signal arrived); when wrapper.py spawns its `subprocess.call(shell_cmd, ..., preexec_fn=_child_ignore_sigterm)` the preexec_fn re-installs SIG_IGN in the child so the inherited disposition for the subprocess bash and everything it execs is SIG_IGN. wrapper.py itself stays alive because its handler swallows the signal cleanly. The handler-vs-SIG_IGN split is load-bearing: wrapper.py needs to *observe* the signal to make the success-marker decision later, while the chain underneath needs SIG_IGN inheritance so the subprocess shell does not deferred-terminate exit 143 and bury the user's status.
- **apptainer is a hard signal barrier** that magnus does not assume away. apptainer 1.x installs its own SIGTERM handler with no flag to disable it; on receiving SIGTERM it tears the container down and exits ~143. magnus does **not** depend on apptainer's specific version or its internal handler behavior — apptainer is allowed to die, the chain above and below it adapts.
- **Inside the container the user process inherits SIG_DFL** for SIGTERM (apptainer either forwards the signal or wipes the disposition; either way the in-container chain receives it as default). Without a handler the process is terminated; with a `signal.signal(SIGTERM, …)` handler the user code runs its own teardown and decides its own exit code.

**SLURM path (terminate / preemption)**: `kill_job` uses `scancel --signal=KILL --full` to deliver SIGKILL to all PIDs immediately (SIGKILL cannot be ignored at the kernel level), followed by a bare `scancel <jobid>` to flip the SLURM job state to CANCELLED. Falling back to the default `scancel <jobid>` (KillSignal=SIGTERM, KillWait grace before SIGKILL) would waste the entire grace period spinning on the SIG_IGN'd half of the chain, breaking preemption's "yield GPUs immediately" promise. SIGKILL-direct keeps terminate and preemption latencies in the seconds range.
...
2. `.magnus_user_script.sh` (rendered by `_render_docker_user_script` in `_scheduler/_submit.py`) places the user's `entry_command` inside `( set -e; ... ) &` — a backgrounded subshell whose disposition is SIG_DFL. When the subshell's last command is a single long-running process (the typical `pip install ... && python train.py` pattern) bash exec-tail-calls the subshell PID into that process; SIG_DFL passes through. The outer bash installs `trap 'kill -TERM "$_magnus_pid"' TERM` plus a `while ... wait` loop that re-waits after signal-interrupted waits (which return 128+sig) and propagates the child's real exit status through `exit $?`. The outer bash itself does not deferred-terminate because its trap is non-empty.

Together both backends behave the same way at the user-process level: handler-less code is terminated by SIGTERM; handler-aware code runs its handler and chooses its own exit. Deeper nested shells inside `entry_command` would need their own traps to keep relaying — same caveat as SLURM's apptainer.
