...
**SLURM path (signal)**: `scancel --signal=TERM --batch <jobid>` — `--batch` (not `--full`) is the key flag. `--full` would broadcast SIGTERM to every PID in the batch step's cgroup including apptainer's starter, fuse-overlayfs, squashfuse_ll and the rest of the container infrastructure; those have no SIGTERM handler installed and default-disposition-terminate, tearing the squashfs / overlay mount points down and SIGBUS-ing the user process. `--batch` instead targets only the batch step's parent process — wrapper.py — leaving the cgroup intact. wrapper.py then fans out from there, no user-script cooperation, no marker file, no protocol surface:
...
- **User-script SIG_IGN guard**: the rendered `.magnus_user_script.sh` begins with `trap '' TERM`, so the bash that runs the user's `entry_command` inherits `SIG_IGN` for SIGTERM. The wrapper's fan-out signals every NSpid-qualified PID in the cgroup, which includes this bash; without the guard the bash would terminate by default disposition and cascade up to `wrapper.subprocess.call`, dragging the still-handling user process down with the SLURM step teardown before its handler can `sys.exit(0)`. `SIG_IGN` propagates to descendants through POSIX exec; user code that installs a handler via `signal.signal(SIGTERM, …)` / `sigaction(2)` naturally overrides it.
