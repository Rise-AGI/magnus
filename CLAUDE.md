...
### 1. Fast Fail
不要用 try-except 包裹不该包裹的代码。让错误尽早暴露，而非静默吞掉。

### 2. 最小修改原则
**只改动被要求改动的部分**。绝对不要：
- 自作主张"改善"你认为不规范但实际有其道理的代码
- 添加未被要求的功能、重构、注释或类型注解
- 修改与任务无关的代码

### 3. 先理解再动手
**禁止对未读取的代码提出修改建议**。开发前必须：
- 向用户索要组件/模块的最新状态
- 阅读相关现有代码，理解现有模式
- 确认代码上下文同步后再开始开发

### 4. 组件复用优先
前端已有的 UI 组件必须复用，不要重复造轮子。
...
1. **索要最新代码**：修改组件前，先向用户确认当前状态
2. **不要自作主张**：只做被明确要求的修改
3. **复用组件**：前端开发前先查看 `components/ui/` 目录
4. **library 纯净**：library 里不能有 Magnus 相关逻辑
5. **Fast Fail**：避免不必要的 try-except
