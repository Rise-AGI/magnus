...
### 🧬 技能系统 (Skill System)
- **多文件工作区**: 每个技能包含多个文件，支持代码、数据和文档
- **SKILL.md 必须**: 每个技能必须包含描述文件
- **CRUD 管理**: 通过 Web 界面和 CLI 创建、编辑、删除技能
- **权限控制**: Owner 和 Admin 权限管理

### 📦 镜像管理 (Image Management)
- **缓存管理**: 管理集群上的容器镜像缓存
- **异步拉取**: 镜像拉取在后台执行，不阻塞 API
- **安全刷新**: 拉新镜像到临时文件再原子替换，旧镜像在刷新期间保持可用
- **自动恢复**: 服务器重启后自动恢复中断的拉取状态
...
│   │   ├── skills/                                        # 技能相关组件
│   │   │   ├── skill-table.tsx                            # 技能表格
│   │   │   └── skill-editor.tsx                           # 技能编辑器
│   │   ├── layout/                                        # 布局组件
│   │   │   ├── header.tsx                                 # 页面头部
...
│   │   ├── use-editor-state.ts                            # 编辑器状态管理 Hook
...
### 集群概览
- `GET /api/cluster/stats` - 获取集群资源统计
- `GET /api/cluster/my-active-jobs` - 获取当前用户的活跃任务

### 镜像管理
- `GET /api/images` - 列出缓存镜像（合并 DB 记录与 filesystem）
- `POST /api/images` - 注册并拉取新镜像（异步，返回 202）
- `POST /api/images/{id}/refresh` - 重新拉取已缓存镜像（异步，返回 202）
- `DELETE /api/images/{id}` - 删除镜像（DB 记录 + SIF 文件）

### 技能管理
- `GET /api/skills` - 列出技能
- `POST /api/skills` - 创建或更新技能 (upsert)
- `GET /api/skills/{id}` - 获取技能详情（含文件内容）
...
| `pull_image(uri)` | 注册并拉取新镜像（异步） |
| `refresh_image(image_id)` | 重新拉取已缓存镜像（异步） |
| `remove_image(image_id)` | 删除缓存镜像 |
...
# 刷新已缓存镜像（异步，旧镜像在刷新期间保持可用）
magnus image refresh 3
magnus refresh 3               # 顶层快捷方式
...
5. **现代化交互界面**: 基于 Next.js 14 + Tailwind 的响应式控制台，集成飞书用户画像，中英双语支持。
...
10. **国际化 (i18n)**: 中英双语支持，运行时动态切换，所有模块全面中文化。
11. **资源生命周期管理**: 容器镜像与代码仓库的 LRU 缓存淘汰策略。
12. **技能系统 (Skill System)**: 多文件工作区 Skill 管理，支持 CRUD、权限控制、Web 编辑器和 CLI。
13. **镜像管理 (Image Management)**: 容器镜像缓存的完整生命周期管理，异步拉取、安全刷新（原子替换）、自动恢复、CLI 与 Web 界面管理。
14. **集群概览 (Cluster Overview)**: 资源统计、活跃任务、队列深度一站式展示。
15. **人事管理 (People)**: 用户列表、搜索、角色展示。
