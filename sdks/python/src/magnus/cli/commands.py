...
LOCAL_PID_FILE = LOCAL_MAGNUS_DIR / "local_server.pid"
LOCAL_FRONTEND_PID_FILE = LOCAL_MAGNUS_DIR / "local_frontend.pid"
...
def _is_process_alive(pid: int) -> bool:
    """Check if a process is alive, handling cross-platform differences."""
    try:
        os.kill(pid, 0)
        return True
...
def _kill_process(pid: int) -> None:
    """Kill a process gracefully with SIGTERM, escalate to SIGKILL after timeout."""
    import time
    try:
        if sys.platform == "win32":
            os.kill(pid, signal.SIGTERM)  # On Windows, this is TerminateProcess (hard kill)
        else:
        ...
        # Escalate to SIGKILL (Unix only; Windows SIGTERM is already hard kill)
        if sys.platform != "win32":
...
def local_start():
    ...
    if LOCAL_PID_FILE.exists():
        try:
            ...
            if _is_process_alive(pid):
                print_msg(f"Local server already running (PID: {pid})")
                print_msg(f"Use 'magnus local stop' to stop it first.")
                raise typer.Exit(0)
        except ValueError:
            pass
    ...
    # Redirect stderr to a log file instead of PIPE — a PIPE with no reader would
    ...
    LOCAL_PID_FILE.write_text(str(backend_proc.pid))
    ...
    if front_end_path.exists() and (front_end_path / "package.json").exists():
        ...
        if not (front_end_path / "node_modules").exists():
            print_msg("Installing frontend dependencies (first time, may take a minute)...")
        ...
        frontend_env["PORT"] = str(LOCAL_FRONT_END_PORT)
        frontend_env["MAGNUS_CONFIG_PATH"] = str(config_path)
        frontend_env["MAGNUS_DELIVER"] = "TRUE"  # 本地 config 已含最终端口，不需要 +2
        ...
        if frontend_proc.poll() is None:
            LOCAL_FRONTEND_PID_FILE.write_text(str(frontend_proc.pid))
            frontend_pid = frontend_proc.pid
        else:
            print_msg("Warning: Frontend failed to start (backend is still running)")

    # Register as SDK site
    local_address = f"http://127.0.0.1:{LOCAL_BACK_END_PORT}"
    save_site(LOCAL_SITE_NAME, local_address, "local")
    print_msg(f"Backend started (PID: {backend_proc.pid}) at http://127.0.0.1:{LOCAL_BACK_END_PORT}")
    if frontend_pid:
        print_msg(f"Frontend started (PID: {frontend_pid}) at http://localhost:{LOCAL_FRONT_END_PORT}")
    ...
    print_msg("")
    print_msg("You can now use all magnus commands as usual:")
    if frontend_pid:
        print_msg(f"  Open http://localhost:{LOCAL_FRONT_END_PORT} in your browser")
    print_msg("  magnus list              # browse blueprints")
    print_msg("  magnus run <blueprint>   # run a blueprint")
    print_msg("  magnus jobs              # check recent jobs")
...
def local_stop():
    ...
    if LOCAL_PID_FILE.exists():
        try:
            pid = int(LOCAL_PID_FILE.read_text().strip())
            _kill_process(pid)
            print_msg(f"Backend stopped (PID: {pid})")
            stopped_any = True
        except (ProcessLookupError, ValueError):
            print_msg("Backend process not found (already stopped)")
        LOCAL_PID_FILE.unlink(missing_ok=True)

    # Stop frontend
    if LOCAL_FRONTEND_PID_FILE.exists():
        try:
            pid = int(LOCAL_FRONTEND_PID_FILE.read_text().strip())
            _kill_process(pid)
            print_msg(f"Frontend stopped (PID: {pid})")
            stopped_any = True
        except (ProcessLookupError, ValueError):
            print_msg("Frontend process not found (already stopped)")
        LOCAL_FRONTEND_PID_FILE.unlink(missing_ok=True)
...
def local_status():
    ...
    if LOCAL_PID_FILE.exists():
        try:
            pid = int(LOCAL_PID_FILE.read_text().strip())
            backend_alive = _is_process_alive(pid)
            if backend_alive:
                print_msg(f"Backend running (PID: {pid}) at http://127.0.0.1:{LOCAL_BACK_END_PORT}")
            else:
                print_msg("Backend not running (stale PID file)")
                LOCAL_PID_FILE.unlink(missing_ok=True)
        except ValueError:
            print_msg("Invalid backend PID file")

    if LOCAL_FRONTEND_PID_FILE.exists():
        try:
            pid = int(LOCAL_FRONTEND_PID_FILE.read_text().strip())
            frontend_alive = _is_process_alive(pid)
            if frontend_alive:
                print_msg(f"Frontend running (PID: {pid}) at http://localhost:{LOCAL_FRONT_END_PORT}")
            else:
                print_msg("Frontend not running (stale PID file)")
                LOCAL_FRONTEND_PID_FILE.unlink(missing_ok=True)
        except ValueError:
            print_msg("Invalid frontend PID file")
